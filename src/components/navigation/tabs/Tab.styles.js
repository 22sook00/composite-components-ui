...
export const List = styled.ul`
  ...
  border-bottom: ${({ variant, withBorder }) => (variant === 'button' || !withBorder ? `none` : '1px solid #eeeeee')};
  background-color: ${({ variant, bg }) => variant === 'button' && (bg || color.grayscale.gray[100])};
...
export const Tab = styled.li`
  ...
  ::before {
    ...
    border-bottom: ${({ active, c }) =>
      active ? (c ? `2px solid ${c}` : `2px solid ${color.primary.purple[300]}`) : 'none'};
  }
`
...
export const TabLine = styled(BaseButton)`
  height: auto;
  padding: 0px;
  color: ${({ active, c }) => (active ? c || color.primary.purple[300] : '#999999')};
...
export const TabButton = styled(BaseButton)`
  ...
  line-height: 18px;
  font-weight: 500;
  background-color: ${({ active }) => (active ? color.primary.purple[300] : 'transparent')};
